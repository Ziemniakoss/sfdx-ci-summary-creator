...
async function indexFolder(folder: string) {
    const dirContents:{fileOrDir:string, stats}[] = await promises.readdir(folder).then(contents => {
        const fileDataGettingPromises = contents.map(async fileOrDir => {
            try {
                const stats = await promises.lstat(fileOrDir)
    ...
    for (const fileOrDir of dirContents) {
        ...
        if (fileOrDir.stats.isDirectory()) {
            console.log(fileOrDir.fileOrDir, "is dir")
